...
class AbstractImmutableModel(MutableModel):
    ...
    def get_mutability_rule(**kwargs):
        ...
        return MutabilityRule(field_name, values=(ModelState.MUTABLE_STATE,), **kwargs)
    ...
    def saved_value(self, field):
        ...
        hints = {'instance': self}
        db_instance_qs = self.__class__._base_manager.db_manager(hints=hints).filter(pk=self.pk).only(field)
...
class ImmutableRelationModel(AbstractImmutableModel):
    related_field = models.ForeignKey('BaseImmutableModel', on_delete=models.CASCADE, null=True, blank=True)
    mutability_rules = (
        AbstractImmutableModel.get_mutability_rule(field='related_field__state'),
    )


class ImmutableMultiForwardRelModel(AbstractImmutableModel):
    related_field = models.ForeignKey('ModelWithRelation', on_delete=models.CASCADE, null=True, blank=True)
    mutability_rules = (
        AbstractImmutableModel.get_mutability_rule(field='related_field__related_field__state'),
    )


class ImmutableReverseRelationModel(AbstractImmutableModel):
    related_field = models.ForeignKey('ImmutableMultiReverseRelModel', on_delete=models.CASCADE, null=True, blank=True)
    mutability_rules = (
        AbstractImmutableModel.get_mutability_rule(field='modelwithrelation__state'),
    )


class ImmutableMultiReverseRelModel(AbstractImmutableModel):
    mutability_rules = (
        AbstractImmutableModel.get_mutability_rule(field='immutablereverserelationmodel__'
...
class ImmutableMultiMixRelModel(AbstractImmutableModel):
    related_field = models.ForeignKey('BaseImmutableModel', on_delete=models.CASCADE, null=True, blank=True)
    mutability_rules = (
        AbstractImmutableModel.get_mutability_rule(field='related_field__immutablerelationmodel__state'),
        AbstractImmutableModel.get_mutability_rule(field='modelwithrelation__related_field__state')
    )


class ModelWithRelation(AbstractImmutableModel):
    related_field = models.ForeignKey('ImmutableReverseRelationModel', on_delete=models.CASCADE, null=True, blank=True)
    related_field2 = models.ForeignKey('ImmutableMultiMixRelModel', on_delete=models.CASCADE, null=True, blank=True)
